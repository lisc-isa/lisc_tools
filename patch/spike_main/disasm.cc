...
struct : public arg_t{
  std::string to_string(insn_t insn) const{
    return std::to_string( (int)(insn.i_imm() & 0x3f) );
  }
} immx;
/*
struct : public arg_t {
  std::string to_string(insn_t insn) const{
    return std::to_string((int)insn.ij_imm());
  }
} ijmm;
*/
struct : public arg_t {
...
struct : public arg_t {
  std::string to_string(insn_t insn) const {
    return std::to_string(insn.zimm());
  }
} zimm5;
...
disassembler_t::disassembler_t(int xlen)
{
  ...
  //由于位域发生变化，因此对此处进行修改
  const uint32_t mask_rd = 0xfUL << 8;
  const uint32_t match_rd_ra = 1UL << 8;
  const uint32_t mask_rs1 = 0xfUL << 24;
  const uint32_t match_rs1_ra = 1UL << 24;
  const uint32_t mask_rs2 = 0xfUL << 20;
  const uint32_t mask_imm = 0xfffUL << 12;
  const uint32_t match_imm_1 = 1UL << 12;
  // 压缩指令的相关内容，暂时先不改，不清楚如何设计的指令
...
*/
/*
  add_insn(new disasm_insn_t("j", match_jal, mask_jal | mask_rd, {&jump_target})); //不回写的作法
  add_insn(new disasm_insn_t("jal", match_jal | match_rd_ra, mask_jal | mask_rd, {&jump_target})); //默认的X1寄存器用法
  add_insn(new disasm_insn_t("jal", match_jal, mask_jal, {&xrd, &jump_target})); //最普通的最全的用法
*/

  //jal 指令没有相关的宏定义，只能用add_insn 由于我们的指令默认是X1寄存器实现，根本没有RD位域
  add_insn(new disasm_insn_t("j", match_j, mask_j , {&jump_target})); //不回写的作法
  add_insn(new disasm_insn_t("jal", match_jal, mask_jal, {&jump_target}));
  //??????????
  ...
  add_insn(new disasm_insn_t("blez" , match_bge , mask_bge | mask_rs1 , {&xrs2 , &branch_target}));
  add_insn(new disasm_insn_t("ble" ,match_bge , mask_bge ,{&xrs2 , &xrs1 ,&branch_target}));
  add_insn(new disasm_insn_t("bleu" ,match_bgeu , mask_bgeu ,{&xrs2 , &xrs1 ,&branch_target}));
  add_insn(new disasm_insn_t("bgtz" , match_blt , mask_blt | mask_rs1 , {&xrs2 , &branch_target}));
  add_insn(new disasm_insn_t("bgt" , match_blt ,mask_blt , {&xrs2 , &xrs1 ,&branch_target}));
  add_insn(new disasm_insn_t("bgtu" , match_bltu ,mask_bltu , {&xrs2 , &xrs1 ,&branch_target}));
  ...
  //由于指令的立即数位域不一致，因此所有的宏定义失效，重写
  //不用 目的寄存器的 使用jalr，写回目的寄存器的用jalrl
  add_insn(new disasm_insn_t("ret", match_jalr | match_rs1_ra, mask_jalr | mask_rd | mask_rs1 | mask_imm, {}));
  add_insn(new disasm_insn_t("jr", match_jr, mask_jr ,{&xrs1,&imm}));
  add_insn(new disasm_insn_t("jr", match_jr, mask_jr ,{&xrs1}));

  add_insn(new disasm_insn_t("jalr", match_jalr | match_rd_ra, mask_jalr | mask_rd | mask_imm, {&xrs1}));
  add_insn(new disasm_insn_t("jalr", match_jalr | match_rd_ra, mask_jalr | mask_rd , {&xrs1 , &imm}));
  add_insn(new disasm_insn_t("jalr", match_jalr , mask_jalr | mask_imm , {&xrd , &xrs1}));
  add_insn(new disasm_insn_t("jalr",match_jalr , mask_jalr , {&xrd ,&xrs1 , &imm}));
...
//  DEFINE_ITYPE(srai);
  ...
  add_insn(new disasm_insn_t("slli" , match_slli , mask_slli ,{&xrd , &shamt} ) );
  add_insn(new disasm_insn_t("srli" , match_srli , mask_srli ,{&xrd , &shamt} ) );
  add_insn(new disasm_insn_t("srai" , match_srai , mask_srai ,{&xrd , &shamt} ) );
...
*/
  ...
  DEFINE_RTYPE(add);
  DEFINE_RTYPE(sub);
  add_insn(new disasm_insn_t("neg",match_sub , mask_sub | mask_rs1 ,{&xrd , &xrs2}));
  DEFINE_RTYPE(sll);
  DEFINE_RTYPE(slt);
...
*/
  ...
  add_insn(new disasm_insn_t("rdcycle", match_rdcycle , mask_rdcycle ,{&xrd}));
  add_insn(new disasm_insn_t("rdinstret", match_rdinstret , mask_rdinstret ,{&xrd}));
  add_insn(new disasm_insn_t("rdtime", match_rdtime , mask_rdtime ,{&xrd}));
  add_insn(new disasm_insn_t("rdcycleh", match_rdcycleh , mask_rdcycleh ,{&xrd}));
  add_insn(new disasm_insn_t("rdinstreth", match_rdinstreth , mask_rdinstreth ,{&xrd}));
  add_insn(new disasm_insn_t("rdtimeh", match_rdtimeh , mask_rdtimeh ,{&xrd}));
...
//  add_insn(new disasm_insn_t("unimp", match_csrrw | (CSR_CYCLE << OP_SH_CSR), 0xffffffffU,{}));
